...
class procedure TTransporter.LoadTexture(aAtlas: TOGLCTextureAtlas; aAdditionalScale: single);
var dataFolder: string;
begin
  FAdditionalScale := aAdditionalScale;
  dataFolder := FScene.App.DataFolder;
  texTransporterBody := aAtlas.AddFromSVG(dataFolder+'Sprites\WolfCastle\TransporterBody.svg', ScaleW(1040), -1);
  texTransporterLeg := aAtlas.AddFromSVG(dataFolder+'Sprites\WolfCastle\TransporterLeg.svg', -1, ScaleH(180));
  texTransporterPlatform := aAtlas.AddFromSVG(dataFolder+'Sprites\WolfCastle\TransporterPlatform.svg', ScaleW(498), -1);
  texTransporterActuator := aAtlas.AddFromSVG(dataFolder+'Sprites\WolfCastle\TransporterActuator.svg', -1, ScaleH(47));
end;

procedure TTransporter.SetFlipH(AValue: boolean);
begin
...
procedure TTransporter.SetFlipV(AValue: boolean);
begin
...
constructor TTransporter.Create(aLayerIndex: integer);
begin
...
procedure TTransporter.ProcessMessage(UserValue: TUserMessageValue);
begin
  case UserValue of
    0: begin
    end;
...
procedure TTransporter.Posture_IdleOnGround(aDuration: single);
begin
...
procedure TTransporter.Posture_OnlyLegs(aDuration: single);
begin
  LeftLeg.MoveTo(0.081*TransporterBody.Width, 0.858*TransporterBody.Height, aDuration, idcSinusoid);
  LeftLeg.Angle.ChangeTo(0, aDuration, idcSinusoid);
  LeftLeg.Scale.ChangeTo(PointF(1.0, 1.0), aDuration, idcSinusoid);
...
procedure TTransporter.Posture_HalfPlatformOut(aDuration: single);
begin
  LeftLeg.MoveTo(0.081*TransporterBody.Width, 0.858*TransporterBody.Height, aDuration, idcSinusoid);
  LeftLeg.Angle.ChangeTo(0, aDuration, idcSinusoid);
  LeftLeg.Scale.ChangeTo(PointF(1.0, 1.0), aDuration, idcSinusoid);
...
procedure TTransporter.Posture_InAir(aDuration: single);
begin
